...
/* Donut palette — indigo/violet family used for the "Leads by Source" chart. */
const SOURCE_COLORS = ["#4f46e5", "#818cf8", "#312e81", "#a5b4fc", "#4338ca", "#c7d2fe"];
...
function EngagementChart({ trend }) {
  ...
  const renderPeak = (props) => {
    ...
    return (
      <g>
        <circle cx={cx} cy={y} r={5} fill="#312e81" stroke="#fff" strokeWidth={2} />
        <rect x={cx - 26} y={y - 34} width={52} height={22} rx={11} fill="#312e81" />
    ...
    <ResponsiveContainer width="100%" height={280}>
      <BarChart data={trend} barCategoryGap="28%" margin={{ top: 30 }}>
        ...
        <Bar dataKey="leads" radius={[14, 14, 14, 14]} maxBarSize={42} label={renderPeak}>
          {trend.map((t, i) => (
            <Cell key={i} fill={i === maxIndex ? "#4338ca" : "#c7d2fe"} />
          ))}
        </Bar>
      </BarChart>
    </ResponsiveContainer>
  ...
  return (
    <ResponsiveContainer width="100%" height={120}>
      <AreaChart data={trend} margin={{ top: 14, right: 0, left: 0, bottom: 0 }}>
        ...
        <Area type="monotone" dataKey="won" stroke="#4f46e5" strokeWidth={2.5} fill="url(#balance)" />
      </AreaChart>
    </ResponsiveContainer>
  );
}
